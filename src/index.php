<?php
session_start(); // Toujours tout en haut, sans espace avant
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Vérifie si l'utilisateur a déjà donné un consentement
    const consent = localStorage.getItem("rgpdConsent");
    if (!consent) {
        // Premier accès : on bloque le site et on affiche le modal
        blockSite();
        modal.show();
        ...
        document.getElementById("rgpdRefuse").addEventListener("click", function () {
            ...
            showMandatoryPopup(); // message immédiat
        });
    }
});
</script>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark mb-5 shadow-sm">
    <div class="container">
        ...
        <?php if(isset($_SESSION['login'])): ?>
            ...
            <a class="nav-link btn btn-outline-primary" style="color: lightgrey; padding: 0.5rem 0.5rem;" href="index.php?module=compte&action=solde&id=<?php echo $_SESSION['id_compte']; ?>">Votre Solde</a>
        <?php endif; ?>
